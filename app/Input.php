...
{
    protected $fillable = [
        'ip', 'phone', 'date', 'hour', 'state', 'delayed', 'user_id', 'created_at'
    ];
    ...
    public static function timeRetUser(){
        $agencies = DB::table('users')
        ->join('inputs', 'users.id', '=', 'inputs.user_id')
        ->select('users.name', DB::raw('sum("inputs"."delayed") as "total_delayed"'))
        ->groupBy('users.name')
        ->get();
        return $agencies;
    }

    public static function timeRetAge($id){
        $agencies = DB::table('users')
        ->join('inputs', 'users.id', '=', 'inputs.user_id')
        ->where('users.office_id', $id)
        ->select('users.name', DB::raw('sum("inputs"."delayed") as "total_delayed"'))
        ->groupBy('users.name')
        ->get();
        return $agencies;
